...
class AudioSealWatermarker:
    ...
    def embed(self, wav: bytes) -> bytes:
        ...
        # Inference only: no autograd graph, so the result can go straight to numpy
        # (the generator's output otherwise requires grad and .numpy() would raise).
        with torch.no_grad():
            watermarked = self._generator(tensor, sample_rate=rate, alpha=1.0) + tensor
        out = (
            watermarked.squeeze().clamp(-1.0, 1.0).detach().cpu().numpy() * 32767.0
        ).astype("<i2").tobytes()
